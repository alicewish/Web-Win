...
/*
@@@BUILDINFO@@@ Script Events Manager.jsx 1.0.0.3
*/
...
<about>$$$/JavaScripts/ScriptEventsManager/About=Script Events Manager ^r^rCopyright 2007 Adobe Systems Incorporated. All rights reserved.^r^rAllows you to manage the current list of notifiers running.</about>
	...
	ddFile.onChange = function () {
		if ( gCustomFileIndex == this.selection.index ) {
			...
			if ( null != newFile ) {
				...
				UpdateDescriptionFromFile( newFile );
			} else {
				ddFile.items[ gLastFileIndex ].selected = true;
			}
		} else {
			gLastFileIndex = this.selection.index;
			UpdateDescriptionFromFile( gScriptsFiles[ this.selection.index ] );
		}
...
function InitParams( inObject ) {
	...
	inObject['events'].push( new EventData( strSaveDocument, 'save' ) );
...
function CreateNewEvent() {
	...
	dlgNewEvent.alignChildren = 'left';
...
//        data from the presets/scripts/event scripts only
	...
	if ( undefined == newFile ) {
		...
		if ( allFiles.length > 0 ) {
			...
			for ( var i = 0; i < allFiles.length; i++ ) {
				if ( allFiles[i].name.substr( 0, strActionNotifier.length ) != strActionNotifier && 
					...
					if ( true == allFiles[i].alias ) {
						dd.add( "item", File.decode( allFiles[i].resolve().name ) );
					} else {
						dd.add( "item", File.decode( allFiles[i].name ) );
					}
		...
		if ( undefined != gExtraFiles ) {
			for ( var i = 0; i < gExtraFiles.length; i++ ) {
				dd.add( "item", File.decode( File( gExtraFiles[i] ).name ) );
		...
		if ( true == newFile.alias ) {
			dd.add( "item", File.decode( newFile.resolve().name ) );
		} else {
			dd.add( "item", File.decode( newFile.name ) );
		}
		dd.items[ dd.items.length - 1 ].selected = true;
...
///////////////////////////////////////////////////////////////////////////////
// Function: UpdateDescriptionFromFile
// Usage: update the description static text field with data from the file
//        currently only displays the file name
// Input: file to read out a description
///////////////////////////////////////////////////////////////////////////////
function UpdateDescriptionFromFile( inFile ) {
	var dataSet = false;
	if ( undefined != inFile ) {
		if ( undefined == gOneTime ) {
			gOneTime = true;
		} else {
			var readFile = inFile;
			...
			if ( readFile.open( "r" ) ) {
				...
				readFile.close();
				var stringStartNew = 'var begDesc = "';
				var stringEndNew = '" // endDesc';
				...
				if ( descriptionStart != -1 && descriptionEnd != -1 && descriptionEnd > descriptionStart ) {
					stDescription.text = localize( readData.substr( descriptionStart, descriptionEnd - descriptionStart ) );
					dataSet = true;
				}
				if ( ! dataSet ) {
					var stringStartOld = '/** "';
